...
RUN cp -r ./${git_repository}.wiki/. ./${git_repository}/docs/

RUN cd /${git_repository}/website/ && yarn install && yarn build && cp -r ./build/ ../../build/ && cd ../..

RUN git clone https://github.com/${git_root}/${dist_git_repo}.git && cd ./build && cp -r . ../${dist_git_repo}/ && cd ../${dist_git_repo} &&\
        git config user.email ${git_user_email} && touch somefile.js && git add . && git commit -m $(git rev-parse HEAD) && \
